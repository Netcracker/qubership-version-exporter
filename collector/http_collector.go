	...
	for _, metric := range metrics {
		...
		if metric.JsonPath != "" {
			...
			if err = jPath.Parse(metric.JsonPath); err != nil {
				_ = level.Error(httpCollector.logger).Log("msg", "Error occurred while parsing jsonpath", "jsonpath: ", metric.JsonPath, "err", err)
				return nil, err
			}
	...
	for _, req := range httpCollector.requests {
		...
		go func(req *HttpRequest) {
			...
			if err != nil {
				...
				_ = level.Error(httpCollector.logger).Log("Error from scraper", httpCollector.Name()+req.path, "err", err)
...
func (httpCollector *HttpCollector) doScrape(req *HttpRequest, ch chan<- prometheus.Metric) error {
	...
	default:
		_ = level.Error(httpCollector.logger).Log("msg", "Wrong request method", "method", req.method)
		return nil
	}
	if err != nil {
		_ = level.Error(httpCollector.logger).Log("msg", "Failed to create HTTP request", "err", err)
		return nil
	}
	...
	if err != nil {
		_ = level.Error(httpCollector.logger).Log("msg", "Response is bad", "err", err)
		return nil
	}
	if res.StatusCode != 200 {
		_ = level.Error(httpCollector.logger).Log("msg", "Response status code is not acceptable", "statusCode", res.StatusCode)
		_ = res.Body.Close()
		return nil
	}
	...
	if err = res.Body.Close(); err != nil {
		_ = level.Error(httpCollector.logger).Log("msg", "Error occurred when closing response body", "err", err)
		return nil
	}
...
func (httpCollector *HttpCollector) parseResponse(res *http.Response, metrics []HttpMetric) (metricLabels [][]Labels) {
	...
	if res != nil && res.Body != nil {
		...
		if err != nil {
			_ = level.Error(httpCollector.logger).Log("msg", "Error while reading response body", "err", err)
			return
		}
		...
		if contentType != "" && strings.Contains(contentType, "application/json") {
			...
			if err = json.Unmarshal(body, &responseJson); err != nil {
				_ = level.Error(httpCollector.logger).Log("msg", "Error while parsing response body", "err", err)
				return
			}
			...
			_ = level.Error(httpCollector.logger).Log("msg", fmt.Sprintf("Unsupported content type: %s", contentType))
		}
	}
		...
		if metric.jsonPath != nil {
			...
			if isRange {
				...
				if err = jPath.Parse(metric.jsonPathStr); err != nil {
					_ = level.Error(httpCollector.logger).Log("msg", "Error occurred while parsing jsonpath", "jsonpath: ", metric.jsonPathStr, "err", err)
					continue
				}
				jPath.EnableJSONOutput(true)
			...
			if err = metric.jsonPath.Execute(value, responseJson); err != nil {
				_ = level.Error(httpCollector.logger).Log("msg", fmt.Sprintf("Error occurred while getting value. jsonpath : %s", metric.jsonPathStr), "err", err)
			...
			if len(result) == 1 && result[0] == metric.jsonPathStr || emptyResult {
				_ = level.Debug(httpCollector.logger).Log("msg", fmt.Sprintf("Nothing found. The jsonpath: %s", metric.jsonPathStr))
				result = nil
				continue
			}
			...
			if resultSize%numberOfLabels != 0 {
				_ = level.Error(httpCollector.logger).
					Log("msg",
			...
			_ = level.Error(httpCollector.logger).Log("msg",
				"Invalid configuration. Response type is: 'application/json'. JsonPath can't be empty in the configuration")
		}
	}
...
func (request *HttpRequest) sendMetrics(metricLabels [][]Labels, ch chan<- prometheus.Metric, logger log.Logger) {
...
func (httpCollector *HttpCollector) getCredentialsAndCertificates(config collectorModel.Connector, ctx context.Context) (user *User, tlsConf *tls.Config, err error) {
	...
	if config.Credentials.User.Name != "" && config.Credentials.Password.Name != "" {
		...
		if err != nil {
			_ = level.Error(httpCollector.logger).Log("msg", "can't get authorization data", "err", err)
			return nil, nil, err
		}
		...
		if config.Credentials.User.Name != config.Credentials.Password.Name {
			...
			if err != nil {
				_ = level.Error(httpCollector.logger).Log("msg", "can't get secret data", "err", err)
				return nil, nil, err
			}
		} else {
	...
	if tlsConfig.TLSSkip {
		...
		if !ok {
			_ = level.Error(httpCollector.logger).Log("msg", "can't parse Certificate Authority", "err", err)
			return nil, nil, err
		}
		if pkey != "" && certValue != "" {
			clientCert, err := tls.X509KeyPair([]byte(certValue), []byte(pkey))
			if err != nil {
				_ = level.Error(httpCollector.logger).Log("msg", "can't parse certificate", "err", err)
				return nil, nil, err
			}
			tlsConf = &tls.Config{
				RootCAs:      caCertPool,
