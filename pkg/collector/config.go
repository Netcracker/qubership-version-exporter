...
import (
	...
	configMapModel "github.com/Netcracker/qubership-version-exporter/pkg/model/configmap"
	httpModel "github.com/Netcracker/qubership-version-exporter/pkg/model/http"
	"github.com/Netcracker/qubership-version-exporter/pkg/model/postgres"
	"github.com/Netcracker/qubership-version-exporter/pkg/model/ssh"
...
func NewConfigContainer(configPath, namespace string, clientSet kubernetes.Interface, logger slog.Logger) *Container {
...
func (c *Container) ReadConfig(ctx context.Context) error {
	c.logger.Debug(fmt.Sprintf("trying to read config: %s", c.ConfigPath))
	...
	if err = yaml.UnmarshalStrict(yamlFile, &configMap); err != nil {
		c.logger.Error("failed to decode config", "error", err)
		return err
	}
	...
	for cType, rawConfig := range configMap {
		if rawConfig.unmarshal == nil {
			return errs.Errorf("%s config cannot be unmarshaled", cType.String())
		} else {
			switch cType {
			case Postgres:
...
func (c *Container) validateConfig(ctx context.Context, config interface{}, configType string) error {
	if err := validation.Validator().Struct(config); err != nil {
		...
		if ok {
			c.logger.Error(fmt.Sprintf("config %s isn't valid", configType), "error", validationErrors.Translate(validation.Translator()))
		} else {
			c.logger.Error(fmt.Sprintf("config %s isn't valid", configType))
		}
