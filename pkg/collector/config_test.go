...
func TestLoadingEmptyModuleConfig(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "postgres_collector config cannot be unmarshaled", err.Error())
	assert.Equal(t, 0, len(cfg.CollectorConfigs))

	cfg, err = loadTestConfig(ctx, "emptyModule.yaml", httpCollectorConfigPath)
	assert.Error(t, err)
	assert.Equal(t, "http_collector config cannot be unmarshaled", err.Error())
	assert.Equal(t, 0, len(cfg.CollectorConfigs))

	cfg, err = loadTestConfig(ctx, "emptyModule.yaml", configmapCollectorConfigPath)
	assert.Error(t, err)
	assert.Equal(t, "configmap_collector config cannot be unmarshaled", err.Error())
	assert.Equal(t, 0, len(cfg.CollectorConfigs))
...
func TestLoadingValidSshConfig(t *testing.T) {
	...
	assert.Equal(t, "1.2.3.4", sshConnections.Connections[0].Host)
	...
	assert.Equal(t, "2.3.4.5", sshConnections.Connections[1].Host)
...
func TestLoadingEmptySshModuleConfig(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "ssh_collector config cannot be unmarshaled", err.Error())
	assert.Equal(t, 0, len(cfg.CollectorConfigs))
}
