...
func newPostgresCollector(logger slog.Logger) (Collector, error) {
...
func (spv *PostgresVersionScraper) Scrape(ctx context.Context, metrics *Metrics, ch chan<- prometheus.Metric) error {
		...
		go func(pgClient *PostgresClient) {
			...
			if errs != nil {
				...
				for _, err := range errs {
					spv.logger.Error(fmt.Sprintf("Error from scraper: %s %s", spv.Name(), pgClient.Url), "error", err)
	...
	spv.logger.Debug("Done", "type", Postgres.String())
	return nil
}
...
func (spv *PostgresVersionScraper) InitPostgres(ctx context.Context, connOptions postgres.ConnOptions) *PostgresClient {
	...
	if err != nil {
		spv.logger.Error("can't get pg user data", "error", err)
		return nil
	}
	...
	if connOptions.Credentials.User.Name != connOptions.Credentials.Password.Name {
		...
		if err != nil {
			spv.logger.Error("can't get pg secret data", "error", err)
			return nil
		}
	} else {
	...
	if err != nil {
		spv.logger.Error("can't create postgres client", "error", err)
		return nil
	}
		...
		for _, metric := range req.Metrics {
			...
			if metric.Label != "" {
				if model.UTF8Validation.IsValidLabelName(metric.Label) {
					...
					spv.logger.Error(fmt.Sprintf("label name - %s is not a valid", string(*label)))
					return nil
				}
			}
...
func (spv *PostgresVersionScraper) InitPostgresClient(ctx context.Context, pgUri string, timeout model.Duration) (*PostgresClient, error) {
	...
	// TODO: do we need custom logger here?
	//cfg.ConnConfig.Logger = kitlogadapter.NewLogger(spv.logger.With("pg_caller"))
	...
	spv.logger.Info(fmt.Sprintf("Successfully connected to Postgres. Version: %s", version))
