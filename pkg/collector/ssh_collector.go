...
import (
	...
	sshModel "github.com/Netcracker/qubership-version-exporter/pkg/model/ssh"
...
func newSSHCollector(logger slog.Logger) (Collector, error) {
...
func (svc *SshVersionScraper) InitSshClient(ctx context.Context, connOptions sshModel.ConnOptions) *SshClient {
	...
	if connOptions.Credentials != nil {
		...
		if err != nil {
			svc.logger.Error("can't get authorization data", "error", err)
			return nil
		}
	...
	if connOptions.K8sCredentials != nil {
		...
		if err != nil {
			svc.logger.Error("can't get login user data", "error", err)
			return nil
		}
		...
		if connOptions.K8sCredentials.Login.Name != connOptions.K8sCredentials.PKey.Name {
			...
			if err != nil {
				svc.logger.Error("can't get pkey data", "error", err)
				return nil
			}
		} else {
		...
		if err != nil {
			svc.logger.Error("can't get authorization method", "error", err)
			return nil
		}
	}
	...
	if err != nil {
		svc.logger.Error(fmt.Sprintf("can't connect to the SSH server, address: %s", addr), "error", err)
		return nil
	}

	err = client.Close()
	if err != nil {
		svc.logger.Error(fmt.Sprintf("failed to close client connection to SSH server, address: %s", addr), "error", err)
		return nil
	}
	...
	if err != nil {
		svc.logger.Error("failed to initialize ssh cmd request", "error", err)
		return nil
	}
...
func (svc *SshVersionScraper) Scrape(ctx context.Context, metrics *Metrics, ch chan<- prometheus.Metric) error {
		...
		go func(sshClient *SshClient) {
			...
			if errs != nil {
				...
				for _, err := range errs {
					svc.logger.Error(fmt.Sprintf("Error from scraper: %s %s", svc.Name(), sshClient.Address), "error", err)
	...
	svc.logger.Debug("Done", "type", SSHType.String())
	return nil
}
